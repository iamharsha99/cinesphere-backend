...
// Controller to get watchlist with pagination, sorting, and filtering
const getWatchlist = async (req, res) => {
  ...
  try {
    const [movies] = await req.db.query('CALL GetWatchlist(?, ?, ?, ?, ?)', [userId, limit, offset, sort, filter]);
    const [countResult] = await req.db.query('SELECT COUNT(*) AS count FROM watchlist WHERE user_id = ?', [userId]);
