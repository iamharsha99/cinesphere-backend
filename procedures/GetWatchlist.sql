...
CREATE OR REPLACE PROCEDURE GetWatchlist(IN userId INT, IN in_limit INT, IN in_offset INT, IN in_sort VARCHAR(20), IN in_filter VARCHAR(20))
BEGIN
  ...
  SET @filterCondition = CASE in_filter
    ...
    WHEN 'recent' THEN 'w.created_at >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)'
    ELSE '1=1'
  END;
  ...
  SET @query = CONCAT('
    ...
    WHERE w.user_id = ? AND ', @filterCondition, '
    ORDER BY ', @sortColumn, '
    LIMIT ? OFFSET ?');
